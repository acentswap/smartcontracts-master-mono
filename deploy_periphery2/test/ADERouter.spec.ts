...
import IADEPair from '../build/IADEPair.json'
...
describe('ADERouter', () => {
  ...
  it('getAmountOut', async () => {
    ...
    await expect(router.getAmountOut(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100))).to.be.revertedWith(
      'ADELibrary: INSUFFICIENT_INPUT_AMOUNT'
    )
  ...
  it('getAmountIn', async () => {
    ...
    await expect(router.getAmountIn(BigNumber.from(0), BigNumber.from(100), BigNumber.from(100))).to.be.revertedWith(
      'ADELibrary: INSUFFICIENT_OUTPUT_AMOUNT'
    )
  ...
  it('getAmountsOut', async () => {
    ...
    await expect(router.getAmountsOut(BigNumber.from(2), [token0.address])).to.be.revertedWith('ADELibrary: INVALID_PATH')
  ...
  it('getAmountsIn', async () => {
    ...
    await expect(router.getAmountsIn(BigNumber.from(1), [token0.address])).to.be.revertedWith('ADELibrary: INVALID_PATH')
    const path = [token0.address, token1.address]
  ...
  describe('fee-on-transfer tokens', () => {
    ...
    beforeEach(async function() {
      ...
      pair = new Contract(pairAddress, JSON.stringify(IADEPair.abi), provider).connect(wallet)
    })
