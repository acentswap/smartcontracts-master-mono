...
const Supercluster = artifacts.require("Supercluster");
const truffleAssert = require("truffle-assertions");

contract("Supercluster", ([alice, bob, carol, admin, minter]) => {
  let lp;
  let superclusterToken;
  let contract;
  let snapshotA;
  let startBlock;
  let endBlock;
  ...
  beforeEach(async () => {
    ...
    superclusterToken = await MockERC20.new("Algo", "ALGO", "1000000", {
      from: minter,
    });
    ...
    contract = await Supercluster.new(
      lp.address,
      superclusterToken.address,
      startBlock,
      endBlock,
      admin,
      {
  ...
  describe("depositPool", () => {
    ...
    it("should check the start block and end block for depositing", async () => {
      const offeringAmountPool = 100; // Supercluster token
    ...
    it("should calculate the user allocations correctly", async () => {
      ...
      // Receive token from the Supercluster project
      await superclusterToken.transfer(contract.address, offeringAmountPool, {
        from: minter,
      });
  ...
  describe("harvestPool", () => {
    ...
    describe("without tax", () => {
      ...
      it("should harvest and get refund with correct amount when just enough subscription", async () => {
        ...
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "30");
        ...
        assert.equal((await superclusterToken.balanceOf(alice)).toString(), "60");
        ...
        assert.equal((await superclusterToken.balanceOf(carol)).toString(), "90");
      ...
      it("should harvest and get refund with correct amount when over-subscribed", async () => {
        ...
        // Bob after harvest
        assert.equal((await lp.balanceOf(bob)).toString(), "999");
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "16");
        ...
        assert.equal((await superclusterToken.balanceOf(alice)).toString(), "33");
        ...
        assert.equal((await superclusterToken.balanceOf(carol)).toString(), "50");
    ...
    describe("with tax", () => {
      it("should calculate harvest amount, refund and tax correctly when under-subscribed", async () => {
        const offeringAmountPool = 1000; // Supercluster token
      ...
      it("should calculate harvest amount, refund and tax correctly when just enough subscription", async () => {
        ...
        // Bob after harvest
        assert.equal((await lp.balanceOf(bob)).toString(), "950");
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "500");
        ...
        // Alice after harvest
        assert.equal((await lp.balanceOf(alice)).toString(), "980");
        assert.equal((await superclusterToken.balanceOf(alice)).toString(), "200");
        ...
        // Carol after harvest
        assert.equal((await lp.balanceOf(carol)).toString(), "970");
        assert.equal((await superclusterToken.balanceOf(carol)).toString(), "300");
      ...
      it("should calculate harvest amount, refund and tax correctly when 2x subscription", async () => {
        const offeringAmountPool = 10000; // Supercluster token
        ...
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "5000");
        ...
        assert.equal((await superclusterToken.balanceOf(alice)).toString(), "2000");
        ...
        assert.equal((await superclusterToken.balanceOf(carol)).toString(), "3000");
      ...
      it("should calculate harvest amount, refund and tax correctly when 50x subscription", async () => {
        ...
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "4000");
        ...
        assert.equal((await superclusterToken.balanceOf(alice)).toString(), "4000");
        ...
        assert.equal((await superclusterToken.balanceOf(carol)).toString(), "2000");
      ...
      it("should calculate harvest amount, refund and tax correctly when 250x subscription", async () => {
        ...
        assert.equal((await superclusterToken.balanceOf(bob)).toString(), "2000");
  ...
  describe("finalWithdraw", () => {
    ...
    it("should allow to withdraw LP token and offering token correctly", async () => {
      ...
      assert.equal(
        (await superclusterToken.balanceOf(admin)).toString(),
        `${offeringAmountPool}`
      );
    });
  });
  ...
  describe("recoverWrongTokens", () => {
    ...
    it("should not allow to recover offering token", async () => {
      await expectRevert(
        contract.recoverWrongTokens(superclusterToken.address, 1, { from: admin }),
  ...
  describe("viewUserInfo", () => {
    it("should display the user info correctly after depositing into 1 pool, when 2 pools are set", async () => {
      let offeringAmountPool = 100; // Supercluster token
      ...
      offeringAmountPool = 100; // Supercluster token
