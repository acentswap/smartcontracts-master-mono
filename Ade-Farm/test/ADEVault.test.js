...
contract("ADEVault", (accounts) => {
  ...
  beforeEach(async () => {
    adeToken = await ADEToken.new(SUPPLY_PER_YEAR, { from: owner });
    spica = await Spica.new(adeToken.address, { from: owner });
    virgo = await Virgo.new(
      adeToken.address,
      spica.address,
      dev,
      REWARD_START_BLOCK,
    ...
    await adeToken.mint(boredApe, TOKEN_BALANCE, { from: owner });
    await adeToken.mint(penguin, TOKEN_BALANCE, { from: owner });

    await adeToken.transferOwnership(virgo.address, { from: owner });
    await spica.transferOwnership(virgo.address, { from: owner });

    vault = await ADEVault.new(
      adeToken.address,
      spica.address,
      virgo.address,
      owner,
    ...
    await adeToken.approve(vault.address, TOKEN_BALANCE, { from: boredApe });
    await adeToken.approve(vault.address, TOKEN_BALANCE, { from: penguin });
  });
  ...
  describe("deposit", () => {
    it("allows to deposit token when contract is not paused", async () => {
      assert.equal(
        await adeToken.balanceOf(boredApe),
        TOKEN_BALANCE,
    ...
    it("does not allow to deposit when contract is paused", async () => {
      ...
      assert.equal(
        (await adeToken.balanceOf(boredApe)).toString(),
        `${TOKEN_BALANCE - 500}`
      );
    });
  });
  ...
  describe("withdraw", () => {
    ...
    it("allows to withdraw token even when contract is paused", async () => {
      ...
      assert.equal(
        Number.parseInt((await adeToken.balanceOf(boredApe)).toString()),
        TOKEN_BALANCE
      );
    });
  });
  ...
  describe("harvest", () => {
    it("allows to harvest when contract is not paused", async () => {
      ...
      assert.equal(
        Number.parseInt((await adeToken.balanceOf(treasuryAddress)).toString()),
