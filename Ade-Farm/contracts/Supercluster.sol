...
import "./ISupercluster.sol";

contract Supercluster is ISupercluster, ReentrancyGuard, Ownable {
    ...
    // The block number when Supercluster starts
    uint256 public startBlock;

    // The block number when Supercluster ends
    uint256 public endBlock;

    // The campaignId for the Supercluster
    ...
    // Event with campaignId for Supercluster
     ...
     * @param _offeringToken: the token that is offered for the Supercluster
     * @param _startBlock: the start block for the Supercluster
     * @param _endBlock: the end block for the Supercluster
    ...
    constructor(
        ...
        require(_startBlock < _endBlock, "endBlock is less than or equals startBlock");
        require(address(0) != _adminAddress, "set admin address to the zero address");
    ...
    function setPool(
        ...
        require(block.number < startBlock, "Supercluster has started");
    ...
    /**
     * @notice It updates campaignId for the Supercluster.
     * @param _campaignId: the campaignId for the Supercluster
     * @dev This function is only callable by admin.
     */
    function updateCampaignId(
        uint256 _campaignId
    ) external override onlyOwner {
        require(block.number < endBlock, "Supercluster has ended");
